...
final case class Organization(value: String) {
  lazy val parsedValue = LazyProperties.parse(value)
  def map(f: String => String): Organization = {
    val o = LazyProperties.fastApply(value, parsedValue, f)
    if (o eq value) this
    else Organization(o)
  }
...
final case class ModuleName(value: String) {
  ...
  def map(f: String => String): ModuleName = {
    val newName = LazyProperties.fastApply(value, parsedValue, f)
...
final case class Type(value: String) {
  ...
  def nonEmpty: Boolean =
    value.nonEmpty
  private lazy val parsedValue = LazyProperties.parse(value)
  def map(f: String => String): Type = {
    val newValue = LazyProperties.fastApply(value, parsedValue, f)
...
@data class Project(
  ...
  lazy val parsedVersion0 = LazyProperties.parse(version0.asString)
