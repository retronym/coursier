...
/**
 * A sequence of properties, with a mechanism to resolve interpolated references
 * in values.
 *
 * Resolution is lazy.
 *
 ...
 * ```
 *
 * For backwards compatibility, this implements `Seq[(String, String)`, so it can be used
 * in `coursier.core.Project#properties` without making breaking changes to that API.
 * This sequence contains the raw values.
 *
 * `toMap` returns an implementation of `scala.collection.immutable.Map`. Values returned
 * from this map _are_ interpolated, although this is performed lazily.
 *
 * The `LazyProperties.merge` combinators can be used to concatenate multiple properties
 * sequences, typically to merge properties from child/parent layers and to merge intrinsic
 * properties (like `project.version`).
 *
 * These concatenations are themselves lazy, can be chained, and share internal state to
 * enhance performance and reduce memory usage. This state includes an index (`Map[String, Entry`).
 * Each `Entry` caches the parsed representation of the value as a seqeunce of literals
 * and property references. (`ParsedSubstituteProps`)
 *
 ...
 *
 * Many `String`-typed values in the core model can themselves contain property references.
 * Where possible, a class carrying such a field (`value`) should also carry a `lazy val`
 * (`parsedValue`) to cache the parsed representation of that field, and then use
 * `LazyProperties.fastApply(value, parsedValue, f)` in place of `f(value)` in `map` methods.
 * `fastApply` will detect if `f` is of type `LazyProperties.Substitution` and use that
 * to avoid re-parsing the value.
 *
 * If a breaking API change is ever planned, we can cleanup code by: a) wrap all such values in a
 * class that encapsulates the parsed representation, and use this instead of `String`, b) change the
 * type of `Project.properties` to something no longer be a `Seq`, and c) change all the `map(f: String => String)`
 * methods to accept a `map(f: Substitutor)`
 */
private[coursier] sealed abstract class LazyProperties
    extends immutable.AbstractSeq[(String, String)] {
...
private[coursier] object LazyProperties {
  ...
  def merge(layer1: collection.Seq[(String, String)], layer2: PropertyLayer): immutable.Seq[(String, String)] = {
  ...
  private def fillLayers(properties: collection.Seq[(String, String)], dest: Array[PropertyLayer], offset: Int): Int =
    properties match {
  ...
  private final class LazyMapWithSubstitutedValues(
  ...
  ) extends immutable.AbstractMap[String, String] with PropertyValueLookup {
    override def lookupOrNull(key: String): PropertyExpr = owner.winnerEntryForKeyOrNull(key) match {
      case null => null
      case entry =>
        entry.parsed
    }
    ...
    override def removed(key: String): immutable.Map[String, String] =
      immutable.Map.from(entries0).removed(key)
