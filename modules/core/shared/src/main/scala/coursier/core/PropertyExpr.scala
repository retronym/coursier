...
sealed abstract class PropertyExpr {
      ...
      case PropertyReference(name) =>
        ...
        if (resolvedExpr == null) s
          ...
          val result = resolvedExpr match {
            ...
            case spe: SimplePropertyExpr   => PropertyExpr.resolvePartToString(spe, lookup, trim, Nil, 1)
            case Composite(ps)             => PropertyExpr.renderSubstitutions(s, ps, lookup, trim, Nil, forceSubstitute = true, 1)
          }
...
sealed abstract class SimplePropertyExpr extends PropertyExpr
...
object PropertyExpr {
  ...
  final class Substitution(lookup: PropertyValueLookup, trim: Boolean) extends (String => String) {
    override def apply(v1: String): String = applyWithPropertyExpr(v1, parseSubstituteProps(v1))

    def applyWithPropertyExpr(s: String, propertyExpr: PropertyExpr): String = {
      propertyExpr.substitute(s, lookup, trim)
    }
  }
  ...
  private def resolvePartToString(
    ...
    case PropertyReference(name) =>
      ...
      if (resolvedExpr == null) {
        ...
        val result = resolvedExpr match {
          case PropertyLiteral(v)      => v
          case spe: SimplePropertyExpr => resolvePartToString(spe, lookup, trim, nextSeen, depth + 1)
          case Composite(ps)           => renderSubstitutions("", ps, lookup, trim, nextSeen, forceSubstitute = true, depth + 1)
        }
        if (trim) result.trim else result
      }
  }
  ...
  private def appendLiteralToArray(builder: mutable.ArrayBuilder[SimplePropertyExpr], value: String): Unit = {
    if (value.nonEmpty) {
