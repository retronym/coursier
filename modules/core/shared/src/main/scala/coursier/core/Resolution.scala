...
import coursier.version.{ConstraintReconciliation, Version => Version0, VersionConstraint => VersionConstraint0}
...
object Resolution {
  ...
  def substituteProps(s: String, properties: Map[String, String], trim: Boolean): String =
    LazyProperties.substitute(s, properties, trim)
  ...
  final class PropertiesWrapper(val properties: Map[String, String]) {
    val substituteTrimmedProps = new LazyProperties.Substitution(properties, true)
    val substituteProps0 = new LazyProperties.Substitution(properties, false)
  }
  ...
  private def finalDependencies(
    ...
    val parentProperties0 = LazyProperties.concat(
    ...
    val projectWithProperties = withFinalProperties(
      project.withProperties(LazyProperties.concat(parentProperties0, project.properties))
    )
...
@data class Resolution(
  ...
  def dependencyManagementRequirements0(
    ...
    if (needsParent)
      ...
      // 1.1 (see above)
      val approxProperties = LazyProperties.concat(parentProperties0, projectProperties(project))
      ...
      val profileProperties = LazyProperties.concatLayerMaps(
        profiles.map(_.properties)
      )

      val project0 =
        project.withProperties(
          LazyProperties.concat(project.properties, profileProperties)
        ) // belongs to 1.5 & 1.6

      val propertiesMap0 = withFinalProperties(
        project0.withProperties(LazyProperties.concat(parentProperties0, project0.properties))
      ).properties.toMap
      ...
      val modules = withProperties0(
        ...
        new PropertiesWrapper(propertiesMap0)
      ).collect {
  ...
  def withDependencyManagement(project: Project): Project = {
    ...
    val (importDeps, standardDeps) = {
      ...
      val (importDeps0, standardDeps0) = dependencies0
        .map { dep =>
          val (v0, dep0) = withProperties(dep, new PropertiesWrapper(propertiesMap0))
          if (isImport(v0, dep0))
            (dep0 :: Nil, Nil)
          else
    ...
    val importDepsMgmt = {
      ...
      dependenciesMgmt0.flatMap { dep =>
        val (conf0, dep0) = withProperties(dep, new PropertiesWrapper(propertiesMap0))
      ...
      depMgmtInputs ++
        retainedImportProjects.map {
          case (p, endorseStrictVersions) =>
            val overrides = withProperties(p.overrides, new PropertiesWrapper(projectProperties(p).toMap))
    ...
    val substitution = new LazyProperties.Substitution(propertiesMap0, trim = false)
    project0
      .withPackagingOpt(project0.packagingOpt.map(_.map(substitution)))
      .withVersion0(Version0(LazyProperties.substitute(project0.version0.asString, propertiesMap0, trim = false, project0.parsedVersion0)))
      .withDependencies0(
        standardDeps ++
